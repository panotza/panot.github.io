document.onkeypress=getPrintableKey,document.onkeydown=getUniKey;const terminal=document.getElementById("terminal"),history=[];let curPos=0,lastLine=terminal.lastElementChild,i=0,latest=0;const blink=document.createElement("span");blink.classList.add("blink"),history[i]=[],updateScreen();function getPrintableKey(a){"Enter"===a.key||(checkEditing(),history[i].splice(curPos,0,a.key),curPos++,updateScreen())}function checkEditing(){i!==latest&&(history[latest]=[...history[i]],i=latest)}function getUniKey(a){parseKey(a)}function updateScreen(){lastLine.innerHTML="",lastLine.appendChild(document.createTextNode(history[i].slice(0,curPos).join(""))),lastLine.appendChild(blink),lastLine.appendChild(document.createTextNode(history[i].slice(curPos,history[i].length).join(""))),terminal.scrollTop=terminal.scrollHeight}function parseKey(a){switch(a.key){case"Enter":i=i===latest?latest=history.length:latest,history[i]=[],curPos=0,parseCommand(...history[i-1].join("").split(" "));break;case"Backspace":if(0===curPos)return;checkEditing(),history[i].splice(curPos-1,1),curPos--;break;case"ArrowUp":if(a.preventDefault(),0>=i)return;i--,curPos=history[i].length;break;case"ArrowDown":if(a.preventDefault(),i>=history.length-1)break;i++,curPos=history[i].length;break;case"ArrowLeft":if(0>=curPos)return;curPos--;break;case"ArrowRight":if(curPos>=history[i].length)return;curPos++;break;default:return;}updateScreen()}function parseCommand(a,...b){"help"===a?help(...b):"ls"===a?ls(...b):"cat"===a?cat(...b):""===a?println(""):output(`Unknow command ${a}`)}function println(a,b=!1){let c=b?document.createElement("div"):document.createElement("pre");c.innerHTML=a,terminal.appendChild(c),lastLine=c}function output(a){println(a,!0),println("")}function help(){output(`
    <div class="help">
